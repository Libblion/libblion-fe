<template>
  ...
  <header class="dark:bg-gray-950">
    <NavbarUser />
    <div class="flex gap-x-1 z-0 p-4 text-black dark:text-white justify-end">
      <span>Light</span>
      <div class="flex items-center rounded-full">
        <label for="darkMode" class="block w-10 h-5 bg-gray-300 dark:bg-slate-950 rounded-full cursor-pointer relative">
          <input type="checkbox" id="darkMode" class="peer hidden" v-model="isDarkMode" @change="setTheme">
          <div class="w-full h-full rounded-full peer-checked:bg-slate-800"></div>
          <div
